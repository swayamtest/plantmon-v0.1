...
Current runtime implementation temporarily uses:  
`display_name`

as the legacy equivalent of:  
`plant_name`

This naming divergence exists for backward compatibility during migration phases.

Long-term governance direction:  
`plant_name`

Runtime compatibility layers MAY temporarily preserve:  
`display_name`  
...
### **Runtime Maturity Note — repotting\_tasks**

The repotting\_tasks runtime subsystem is currently planned architecture.

Operational separation rules are frozen,  
but full runtime implementation may occur in a later scheduler evolution phase.

Temporary coexistence with legacy repotting task handling is allowed during migration phases.
...
### **Runtime Migration State Note**

The scheduler architecture defined in this section represents:  
the target runtime scheduling model.

Current runtime implementations MAY temporarily continue using:

* static watering\_frequency\_days  
* static fertilizing\_frequency\_days  
* fixed next\_due\_at generation

during progressive scheduler migration phases.

Dynamic seasonal scheduling activation should occur progressively after:  
runtime validation and dataset synchronization complete.
...
Schema migration and infrastructure activation do NOT automatically imply:  
runtime feature activation.

The architecture intentionally supports:  
progressive runtime activation.

Examples:

* canonical identity infrastructure may exist before alias routing activates  
* seasonal fields may exist before dynamic scheduler activation  
* semantic intelligence layers may exist before adaptive notification systems activate
...
## **Transitional Runtime Compatibility Principle**

During progressive migration phases,  
legacy runtime systems MAY temporarily coexist with canonical runtime systems.

Examples include:

* species\_name-based lookups  
* ilike onboarding resolution  
* static scheduler generation  
* legacy task handling

This coexistence is intentional and permitted until:  
runtime validation and progressive activation phases complete.
